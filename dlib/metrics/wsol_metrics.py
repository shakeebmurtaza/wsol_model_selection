...
class LocalizationEvaluator(object):
    ...
    def __init__(self, metadata, dataset_name, split, cam_threshold_list,
                 iou_threshold_list, mask_root, multi_contour_eval, args):
...
class BoxEvaluator(LocalizationEvaluator):
    def __init__(self, **kwargs):
        ...
        self.num_correct_top1_classification = 0
        
        # track raw IoU score
        self.raw_iou_score = np.zeros(len(self.cam_threshold_list))
        
        if self.split == constants.VALIDSET:
            for metadata_roots_pseduo_boxs_valset_name in self.args.metadata_roots_pseduo_boxs_valset.keys():
                metadata_using_pseduo_bboxs = configure_metadata(join(root_dir, self.args.metadata_roots_pseduo_boxs_valset[metadata_roots_pseduo_boxs_valset_name], self.args.dataset, self.split))
                # noise_level = self.args.noise_error_for_eval_with_noisy_bbox[noise_level_indicator]
                setattr(self, f'bboxes_using_{metadata_roots_pseduo_boxs_valset_name}', self._load_resized_boxes(get_bounding_boxes(metadata_using_pseduo_bboxs)))
                
                setattr(self, f'num_correct_using_{metadata_roots_pseduo_boxs_valset_name}',
                {iou_threshold: np.zeros(len(self.cam_threshold_list))
                for iou_threshold in self.iou_threshold_list})
                
                setattr(self, f'num_correct_top1_using_{metadata_roots_pseduo_boxs_valset_name}',
                {iou_threshold: np.zeros(len(self.cam_threshold_list))
                for iou_threshold in self.iou_threshold_list})
                
                setattr(self, f'num_correct_top5_using_{metadata_roots_pseduo_boxs_valset_name}',
                {iou_threshold: np.zeros(len(self.cam_threshold_list))
                for iou_threshold in self.iou_threshold_list})
                
                # track raw IoU score
                setattr(self, f'raw_iou_score_using_{metadata_roots_pseduo_boxs_valset_name}',
                        np.zeros(len(self.cam_threshold_list)))
    ...
    def accumuate_using_pseduo_bboxs(self, image_id, boxes_at_thresholds, number_of_box_list, target, preds_ordered):
        for metadata_roots_pseduo_boxs_valset_name in self.args.metadata_roots_pseduo_boxs_valset.keys():
        # for noise_level_indicator in self.args.noise_level_for_eval_with_noisy_bbox:
            # noise_level = self.args.noise_error_for_eval_with_noisy_bbox[noise_level_indicator]
            multiple_iou = calculate_multiple_iou(
                np.array(boxes_at_thresholds),
                # np.array(self.gt_bboxes[image_id])
                np.array(getattr(self, f'bboxes_using_{metadata_roots_pseduo_boxs_valset_name}')[image_id])
                )  # (nbr_boxes_in_img, 1)
            ...
            raw_iou_score = getattr(self, f'raw_iou_score_using_{metadata_roots_pseduo_boxs_valset_name}')
            raw_iou_score += np.asarray(sliced_multiple_iou)
            ...
            for _THRESHOLD in self.iou_threshold_list:
                ...
                getattr(self, f'num_correct_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD][correct_threshold_indices] += 1
                ...
                if target == preds_ordered[0]:
                    ...
                    getattr(self, f'num_correct_top1_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD][correct_threshold_indices] += 1

                if target in preds_ordered[:5]:
                    # self.num_correct_top5[_THRESHOLD][
                    #     correct_threshold_indices] += 1
                    getattr(self, f'num_correct_top5_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD][correct_threshold_indices] += 1
    ...
    def accumulate(self, scoremap, image_id, target, preds_ordered):
        ...
        #accumulate raw IoU score
        self.raw_iou_score += np.asarray(sliced_multiple_iou)
        ...
        if target == preds_ordered[0]:
            self.num_correct_top1_classification += 1
            
        if self.split == constants.VALIDSET:
            self.accumuate_using_pseduo_bboxs(image_id=image_id, boxes_at_thresholds=boxes_at_thresholds, 
                                number_of_box_list=number_of_box_list, target=target, preds_ordered=preds_ordered)
            
        self.cnt += 1
        
    def compute_using_pseduo_bboxs(self):
        """
        Returns:
        ...
        # for noise_level_indicator in self.args.noise_level_for_eval_with_noisy_bbox:
            # noise_level = self.args.noise_error_for_eval_with_noisy_bbox[noise_level_indicator]
            # max_box_acc = []
            setattr(self, f'max_box_acc_using_{metadata_roots_pseduo_boxs_valset_name}', [])
            # self.best_tau_list = []
            setattr(self, f'best_tau_list_using_{metadata_roots_pseduo_boxs_valset_name}', [])
            
            # self.curve_s = {
            #     'x': self.cam_threshold_list
            # }
            setattr(self, f'curve_s_using_{metadata_roots_pseduo_boxs_valset_name}', {'x': self.cam_threshold_list})
            
            # self.top1 = []
            setattr(self, f'top1_using_{metadata_roots_pseduo_boxs_valset_name}', [])
            
            # self.top5 = []
            setattr(self, f'top5_using_{metadata_roots_pseduo_boxs_valset_name}', [])
            ...
            #     'top5': dict()
            # }
            setattr(self, f'curve_top_1_5_using_{metadata_roots_pseduo_boxs_valset_name}', {
                'x': self.cam_threshold_list,
                'top1': dict(),
                'top5': dict()
            })
            
            setattr(self, f'avg_raw_iou_score_using_{metadata_roots_pseduo_boxs_valset_name}',
                    (getattr(self, f'raw_iou_score_using_{metadata_roots_pseduo_boxs_valset_name}')*100 / self.cnt).max())

            for _THRESHOLD in self.iou_threshold_list:
                # localization_accuracies = self.num_correct[_THRESHOLD] * 100. / \
                #                         float(self.cnt)
                localization_accuracies = getattr(self, f'num_correct_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD] * 100. / \
                                        float(self.cnt)
                # max_box_acc.append(localization_accuracies.max())
                getattr(self, f'max_box_acc_using_{metadata_roots_pseduo_boxs_valset_name}').append(localization_accuracies.max())

                # self.curve_s[_THRESHOLD] = localization_accuracies
                getattr(self, f'curve_s_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD] = localization_accuracies

                # self.best_tau_list.append(self.cam_threshold_list[np.argmax(localization_accuracies)])
                getattr(self, f'best_tau_list_using_{metadata_roots_pseduo_boxs_valset_name}').append(self.cam_threshold_list[np.argmax(localization_accuracies)])

                # loc_acc = self.num_correct_top1[_THRESHOLD] * 100. / float(self.cnt)
                # self.top1.append(loc_acc.max())
                
                loc_acc = getattr(self, f'num_correct_top1_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD] * 100. / float(self.cnt)
                getattr(self, f'top1_using_{metadata_roots_pseduo_boxs_valset_name}').append(loc_acc.max())

                # self.curve_top_1_5['top1'][_THRESHOLD] = deepcopy(loc_acc)
                getattr(self, f'curve_top_1_5_using_{metadata_roots_pseduo_boxs_valset_name}')['top1'][_THRESHOLD] = deepcopy(loc_acc)

                # loc_acc = self.num_correct_top5[_THRESHOLD] * 100. / float(self.cnt)
                # self.top5.append(loc_acc.max())
                
                loc_acc = getattr(self, f'num_correct_top5_using_{metadata_roots_pseduo_boxs_valset_name}')[_THRESHOLD] * 100. / float(self.cnt)                
                getattr(self, f'top5_using_{metadata_roots_pseduo_boxs_valset_name}').append(loc_acc.max())

                # self.curve_top_1_5['top5'][_THRESHOLD] = deepcopy(loc_acc)
                getattr(self, f'curve_top_1_5_using_{metadata_roots_pseduo_boxs_valset_name}')['top5'][_THRESHOLD] = deepcopy(loc_acc)
    ...
    def compute(self):
        ...
        self.avg_raw_iou_score = (self.raw_iou_score*100 / self.cnt).max()
        ...
        self.classfication_acc = self.num_correct_top1_classification * 100. / float(self.cnt)
            
        if self.split == constants.VALIDSET:
            self.compute_using_pseduo_bboxs()
