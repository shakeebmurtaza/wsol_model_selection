...
from dlib.utils.self_learning import BinaryRegionsProposal
...
class CAMComputer(object):
    def __init__(self,
        ...
        if self.args.ablation_model_sele_map_area:
            self.binary_region_gen = BinaryRegionsProposal()
    ...
    def minibatch_accum(self, images, targets, image_ids, image_size) -> None:
        ...
        for image, target, image_id in zip(images, targets, image_ids):
            ...
            # cam: (h, w)
            if self.args.ablation_model_sele_map_area:
                cams_torch = cam.unsqueeze(0).unsqueeze(0)
            cam = t2n(cam)
            ...
            if self.args.ablation_model_sele_map_area:
                binary_cam = self.binary_region_gen(cams_torch).squeeze().cpu().numpy()
                bbox_out = compute_bboxes_from_scoremaps(binary_cam.astype(np.float64), [0.5], multi_contour_eval=False)
                assert len(bbox_out[0]) == 1
                _bbox = np.asarray(bbox_out[0][0])
                
                proposals_area = []
                for bbox in _bbox:
                    area = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
                    #normalize area
                    area = (area / (image_size[0] * image_size[1]))
                    proposals_area.append(area)
                self.binary_region_areas.append(max(proposals_area))
